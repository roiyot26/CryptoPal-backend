...
const getCacheKey = (normalizedKeyword) => `meme_${normalizedKeyword.replace(/\s+/g, '_')}`;
...
const formatMemeResults = (rawMemes = [], keyword) => {
  ...
  return rawMemes
    .map((meme, index) => {
      const image = meme.image || meme.url || meme.picture || meme.thumbnail || null;
...
const fetchMemes = async (displayKeyword, keywordForApi) => {
  ...
  const memeResults =
    data?.memes || data?.results || data?.data || (Array.isArray(data) ? data : []);

  const formattedResults = formatMemeResults(memeResults, keywordForApi);

  if (!formattedResults.length) {
    const noMemesError = new Error(`No memes returned for keyword "${displayKeyword}"`);
...
export const getMemesForUser = async (userPreferences = {}) => {
